...
final readonly class CreateInvitedUserUseCase implements CreateInvitedUserUseCaseInterface
{
    ...
    public function execute(AuthContext $actor, string $email, string $role): InvitedUser
    {
        ...
        if ((new PdoUserRepository($this->query))->findByEmailInOrganization($email, $actor->organizationId) !== null) {
        ...
        $this->transactions->transactional(
            static function (DatabaseQueryExecutorInterface $tx) use ($user, $invitation, $actor): void {
