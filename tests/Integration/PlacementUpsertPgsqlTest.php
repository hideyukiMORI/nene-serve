...
/**
 * Exercises the SqlDialect upsert on real PostgreSQL: `INSERT … ON CONFLICT
 * (keys) DO UPDATE SET … = EXCLUDED.…` with native prepared statements (PG
 * support, issue #120). Skips when no PG is configured.
 */
#[Group('integration')]
final class PlacementUpsertPgsqlTest extends TestCase
{
    ...
    public function testSaveInsertsThenUpsertsOnPostgres(): void
    {
        $repo = new PdoPlacementRepository($this->db, SqlDialect::Pgsql);
