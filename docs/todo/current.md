...
- [~] #3 Provisioning (invite-link via email). Done: mail infrastructure (#86); admin-managed SMTP settings encrypted at rest + test send (#87); **invite-link provisioning backend (PR #88)** — create-user → emailed single-use set-password link → accept → login (verified end-to-end on MySQL + Mailpit). Remaining: **PR C2 frontend** (Users screen + invite, Settings/SMTP screen, set-password page); service-token issuance; first-org bootstrap CLI.
